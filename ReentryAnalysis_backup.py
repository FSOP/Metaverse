...
def main():
    ...
    for reenter_sat in filtered_result:
        if reenter_sat['code'] == 0 and reenter_sat['state_vector'] is not None:
            ...
            result = propagate_hpop_to_surface(reenter_sat['state_vector'], reenter_sat['reentry_time'], force_model)
            ...
            rcs = sat_info['RCS']
            # orbit_crash, inbound_info는 str로 변환 (간단히 repr 사용, 필요시 json 등으로 변환)
            orbit_crash = repr(ephem1)
            time_crash = impact_time
            lat_crash, lon_crash, alt_crash = None, None, None
            ...
            db_man.insert_reentry_event(
                crash_id, creation_date, start_time, norad_cat_id, object_name, object_type, rcs,
                orbit_crash, time_crash, lat_crash, lon_crash, alt_crash, prob_crash, inbound_info_str
            )
